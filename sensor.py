...
# assume all sensor are theshold sensor
...
def get_sensor_theshold(IP, sdr):

    response = raw_command(IP, "0x4 0x27 0x" + sdr[9])
    
    theshold = []
    helper = 1
    for i in range(1, 7):
        if int(response[0], 16) & helper != 0:
            value = analog_convert(sdr, response[i])
            theshold.append(value)
        else:
            theshold.append("N.A.")
        helper <<= 1
    
    theshold[0], theshold[2] = theshold[2], theshold[0]
...
# same as ipmitool sensor
def sensor_print(ID, reading, theshold):
    print("{:14} | {:<20} | {:<25} | {:<8} | {:<8} | {:<8} | {:<8} | {:<8} | {:<8}".format(
            ID, reading[0], reading[1], theshold[0], theshold[1], theshold[2], 
            theshold[3], theshold[4], theshold[5]))
...
def main():
    ...
    print("{:14} | {:<20} | {:<25} | {:<8} | {:<8} | {:<8} | {:<8} | {:<8} | {:<8}".format(
            "ID", "sensor reading", "status", "l.n-r.t.", "l.c.t.", "l.n-c.t.", 
            "u.n-c.t.", "u.c.t.", "u.n-r.t."))
    print("{:{c}^{n}}".format("", c = "-", n = 131))        
    sdr = raw_command(IP, cmd)
    while sdr[0] != "ff" and sdr[1] != "ff":        # FFFFh specifies that the last SDR should be listed
        if sdr[5] == "01":                          # get full sensor only
            ID = sensor_ID(sdr)
            reading = sensor_reading(IP, sdr)
            theshold = get_sensor_theshold(IP, sdr)
            sensor_print(ID, reading, theshold)
        if sdr[5] == "02":                          # get compact sensor
            ID = compact_sensor_ID(sdr)
            reading = sensor_reading(IP, sdr)
            theshold = ["N.A.", "N.A.", "N.A.", "N.A.", "N.A.", "N.A."]
            sensor_print(ID, reading, theshold)
